...
/* ─── Confirm Dialog ─────────────────────────────────────── */
function ConfirmDialog({ title, description, confirmLabel = "Confirm", confirmClassName, onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4" onClick={onCancel}>
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-2">
          <h3 className="text-base font-bold text-[#1c1e21]">{title}</h3>
          {description && <p className="mt-1.5 text-sm text-[#65676b]">{description}</p>}
        </div>
        <div className="flex gap-2 px-6 py-4">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className={confirmClassName ?? "flex-1 rounded-full bg-red-600 py-2 text-sm font-semibold text-white transition hover:bg-red-700"}
          >
            {confirmLabel}
          </button>
        </div>
...
function MemberListView({ chat, currentUser, isOwner, onAddMember, onRemoveMember, onViewProfile }) {
  ...
  const [kickTarget, setKickTarget] = useState(null); // member to confirm kick
    ...
    <>
      ...
      {kickTarget && (
        <ConfirmDialog
          ...
          description={`${kickTarget.displayName} will be removed from this group.`}
          confirmLabel="Kick"
          onConfirm={async () => {
            const target = kickTarget;
            setKickTarget(null);
            try { await onRemoveMember?.(target); }
            catch (err) { window.alert(err.message || "Failed to remove member"); }
          }}
        ...
        <div className="w-full max-w-md rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
          ...
          {deleteConfirm && (
            <ConfirmDialog
              ...
              description={`"${chat.name || "This group"}" and all its messages will be permanently deleted.`}
              confirmLabel="Delete"
